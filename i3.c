...
char *i3_strip_colors( const char *txt )
{
   ...
   for( color = first_i3_color; color; color = color->next )
      strlcpy( tbuf, i3strrep( tbuf, color->i3fish, "" ), LGST );

   for( color = first_i3_color; color; color = color->next )
      strlcpy( tbuf, i3strrep( tbuf, color->i3tag, "" ), LGST );

   for( color = first_i3_color; color; color = color->next )
      strlcpy( tbuf, i3strrep( tbuf, color->mudtag, "" ), LGST );
...
char *I3_mudtofish( const char *txt )
{
   ...
   strlcpy( tbuf, txt, LGST );
   for( color = first_i3_color; color; color = color->next )
      strlcpy( tbuf, i3strrep( tbuf, color->mudtag, color->i3fish ), LGST );
...
char *I3_codetofish( const char *txt )
{
   ...
   strlcpy( tbuf, txt, LGST );
   for( color = first_i3_color; color; color = color->next )
      strlcpy( tbuf, i3strrep( tbuf, color->i3tag, color->i3fish ), LGST );
...
char *I3_codetomud( CHAR_DATA * ch, const char *txt )
{
   ...
   if( I3IS_SET( I3FLAG( ch ), I3_COLORFLAG ) )
   {
      strlcpy( tbuf, txt, LGST );
      for( color = first_i3_color; color; color = color->next )
         strlcpy( tbuf, i3strrep( tbuf, color->i3tag, color->mudtag ), LGST );
   }
   else
      strlcpy( tbuf, i3_strip_colors( txt ), LGST );
...
char *I3_fishtomud( CHAR_DATA * ch, const char *txt )
{
   ...
   if( I3IS_SET( I3FLAG( ch ), I3_COLORFLAG ) )
   {
      strlcpy( tbuf, txt, LGST );
      for( color = first_i3_color; color; color = color->next )
         strlcpy( tbuf, i3strrep( tbuf, color->i3fish, color->mudtag ), LGST );
   }
   else
      strlcpy( tbuf, i3_strip_colors( txt ), LGST );
...
void i3_to_char( const char *txt, CHAR_DATA *ch )
{
   ...
   strlcpy( buf, I3_fishtomud( ch, txt ), LGST );
   strlcpy( buf2, I3_codetomud( ch, buf ), LGST );
...
char *rankbuffer( CHAR_DATA *ch )
{
   ...
   {
      snprintf( rbuf, LGST, "&B%s", CH_CLASSNAME(ch) );

      if( CH_I3RANK(ch) && CH_I3RANK(ch)[0] != '\0' )
         snprintf( rbuf, LGST, "&B%s", CH_I3RANK(ch) );

      if( CH_CLAN(ch) && !strcasecmp( CH_I3NAME(ch), CH_CLANLEADNAME(ch) ) && CH_CLANLEADRANK(ch)[0] != '\0' )
         snprintf( rbuf, LGST, "&B%s", CH_CLANLEADRANK(ch) );

      if( CH_CLAN(ch) && !strcasecmp( CH_I3NAME(ch), CH_CLANONENAME(ch) ) && CH_CLANONERANK(ch)[0] != '\0' )
         snprintf( rbuf, LGST, "&B%s", CH_CLANONERANK(ch) );

      if( CH_CLAN(ch) && !strcasecmp( CH_I3NAME(ch), CH_CLANTWONAME(ch) ) && CH_CLANTWORANK(ch)[0] != '\0' )
         snprintf( rbuf, LGST, "&B%s", CH_CLANTWORANK(ch) );
   }
   return rbuf;
}
...
void I3_process_startup_reply( I3_HEADER * header, char *s )
{
   ...
   /*
    * Receives the router list. Nothing much to do here until there's more than 1 router.
    */
...
void I3_process_ucache_update( I3_HEADER * header, char *s )
{
   ...
   I3_get_field( ps, &next_ps );
   I3_remove_quotes( &ps );
   strlcpy( username, ps, SMST );

   ps = next_ps;
   I3_get_field( ps, &next_ps );
   I3_remove_quotes( &ps );
   strlcpy( visname, ps, SMST );
...
void I3_chan_filter_t( I3_CHANNEL * channel, I3_HEADER * header, char *s )
{
   ...
   I3_get_field( ps, &next_ps );
   I3_remove_quotes( &ps );
   strlcpy( targetmud, ps, SMST );

   ps = next_ps;
   I3_get_field( ps, &next_ps );
   I3_remove_quotes( &ps );
   strlcpy( targetuser, ps, SMST );
...
void I3_process_channel_t( I3_HEADER * header, char *s )
{
   ...
   I3_get_field( ps, &next_ps );
   I3_remove_quotes( &ps );
   strlcpy( message_o, ps, LGST );

   ps = next_ps;
   I3_get_field( ps, &next_ps );
   I3_remove_quotes( &ps );
   strlcpy( message_t, ps, LGST );
...
void I3_process_who_req( I3_HEADER * header, char *s )
{
   ...
   for( d = first_descriptor; d; d = d->next )
   {
      if( d->character && d->connected == CON_PLAYING )
      {
         ...
         strlcpy( rank, rankbuffer( d->character ), 200 );
         ...
         strlcpy( stats, "&z[", 200 );
         if( CH_I3AFK( d->character ) )
            strlcat( stats, "AFK", 200 );
         else
            strlcat( stats, "---", 200 );
         strlcat( stats, "]&G", 200 );

         snprintf( personbuf, LGST, "%s %s%s", stats, CH_I3NAME( d->character ), CH_I3TITLE( d->character ) );
...
void I3_process_finger_reply( I3_HEADER * header, char *s )
{
   ...
   I3_get_field( ps, &next_ps );
   I3_remove_quotes( &ps );
   strlcpy( email, ps, SMST );
   ps = next_ps;

   I3_get_field( ps, &next_ps );
   I3_remove_quotes( &ps );
   strlcpy( last, ps, SMST );
   ps = next_ps;
...
void I3_process_finger_req( I3_HEADER * header, char *s )
{
   ...
   #else
   strlcpy( smallbuf, ctime( CH_LOGON(ch) ), 200 );
   #endif
   I3_write_buffer( smallbuf );
   I3_write_buffer( "\"," );
   ...
   I3_write_buffer( "[PRIVATE]" );
   I3_write_buffer( "\",\"" );
   strlcpy( smallbuf, rankbuffer( ch ), 200 );
   send_to_i3( smallbuf );
   #ifdef FINGERCODE
   I3_write_buffer( "\",\"" );
...
void I3_process_locate_reply( I3_HEADER * header, char *s )
{
   ...
   I3_get_field( ps, &next_ps );
   I3_remove_quotes( &ps );
   strlcpy( status, ps, SMST );

   if( !strcasecmp( status, "active" ) )
      strlcpy( status, "Online", SMST );

   if( !strcasecmp( status, "exists, but not logged on" ) )
      strlcpy( status, "Offline", SMST );
...
void I3_read_packet( void )
{
   ...
   if( I3_input_pointer < (size + 4) )
      return;

   // Extract it into the current packet buffer.
   memcpy( I3_currentpacket, I3_input_buffer + 4, size );

   if( I3_currentpacket[size - 1] == '\r' || I3_currentpacket[size - 1] == '\0' )
   {
      I3_currentpacket[size - 1] = '\0'; // Normalize everything to a null terminator.
   }

   I3_currentpacket[size] = '\0'; // Don't forget to null your strings folks!

   // Shift the remaining data (if any).
   size_t remaining = I3_input_pointer - (size + 4);
   if( remaining > 0 )
   {
      memmove( I3_input_buffer, I3_input_buffer + size + 4, remaining );
   }
   I3_input_pointer = remaining;
}
...
void I3_load_color_table( void )
{
   ...
   i3log( "%s", "Loading I3 color table..." );
...
char *I3_find_social( CHAR_DATA *ch, char *sname, char *person, char *mud, bool victim )
{
   ...
   if( person && person[0] != '\0' && mud && mud[0] != '\0' )
   {
         ...
         && mud && mud[0] != '\0' && !strcasecmp( mud, I3_THISMUD ) )
      {
         ...
         strlcpy( socname, social->others_auto, LGST );
      }
      else
      {
         if( !victim )
         {
            ...
            strlcpy( socname, social->others_found, LGST );
         }
         else
         {
            ...
            strlcpy( socname, social->vict_found, LGST );
         }
      }
   }
   else
   {
      ...
      strlcpy( socname, social->others_no_arg, LGST );
   }
#else
   found  = FALSE;
   ...
   if( person && person[0] != '\0' && mud && mud[0] != '\0' )
   {
         ...
         && mud && mud[0] != '\0' && !strcasecmp( mud, I3_THISMUD ) )
      {
         ...
         strlcpy( socname, social_table[cmd].others_auto );
      }
      else
      {
         if( !victim )
         {
            ...
            strlcpy( socname, social_table[cmd].others_found );
         }
         else
         {
            ...
            strlcpy( socname, social_table[cmd].vict_found );
         }
      }
   }
   else
   {
      ...
      strlcpy( socname, social_table[cmd].others_no_arg );
   }
#endif
   return socname;
}
...
void I3_send_social( I3_CHANNEL * channel, CHAR_DATA * ch, char *argument )
{
   ...
   if( ( socbuf_o && socbuf_o[0] != '\0' ) && ( socbuf_t && socbuf_t[0] != '\0' ) )
   {
      ...
      strlcpy( msg_o, ( char * )i3act_string( socbuf_o, ch, skeleton ), LGST );
      strlcpy( msg_t, ( char * )i3act_string( socbuf_t, ch, skeleton ), LGST );

      if( !skeleton )
         I3_send_channel_emote( channel, CH_I3NAME( ch ), msg_o );
      else
      {
         strlcpy( buf, person, LGST );
         buf[0] = tolower( buf[0] );
